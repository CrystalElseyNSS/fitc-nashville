...
exports.getEventInfo = functions.https.onRequest((request, response) => {
  cors(request, response, async () => {
    events.doc(request.query.id).get().then((doc) => {
        if (doc) {
          return response.status(200).send(doc.data());
        } else {
          return response.status(204).send("event not found");
        }
      })
      .catch((error) => {
        console.log(error);
